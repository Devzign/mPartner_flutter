          ...
          //  height: _subHeaderHeight,
            child: Row(
              children: <Widget>[
                const Spacer(),
                IconButton(
                  icon:  Icon(Icons.chevron_left,color: _isDisplayingFirstMonth?AppColors.whisperGrey:AppColors.lumiBluePrimary,),
                  color: controlColor,
                  tooltip: _isDisplayingFirstMonth ? null : _localizations.previousMonthTooltip,
                  onPressed: _isDisplayingFirstMonth ? null : _handlePreviousMonth,
                ),
                IconButton(
                  icon:  Icon(Icons.chevron_right,color: _isDisplayingLastMonth?AppColors.whisperGrey:AppColors.lumiBluePrimary),
                  color: controlColor,
...
class _DayState extends State<_Day> {
  ...
  Widget build(BuildContext context) {
    ...
    //MaterialStatePropertyAll<Color>(AppColors.lumiBluePrimary)
    final BoxDecoration decoration = widget.isToday
        ? BoxDecoration(
      color:  (widget.isSelectedDay)?AppColors.lumiBluePrimary:dayBackgroundColor,
      border: Border.fromBorderSide(
          (datePickerTheme.todayBorder ?? defaults.todayBorder!)
              .copyWith(color: AppColors.lumiBluePrimary)
      ),
    ...
    Widget dayWidget = Container(
     // padding: EdgeInsets.only(left: 20),
      ...
      child: Center(
        child: Text(localizations.formatDecimal(widget.day.day), style: dayStyle?.apply(color: dayForegroundColor,fontSizeDelta: 2,fontSizeFactor: 0.9)),
      ),
    );
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    if (isCurrentYear) {
      ...
      if (todayBorder != null) {
        border = Border.fromBorderSide(todayBorder.copyWith(color: isSelected?AppColors.lumiBluePrimary:AppColors.lumiBluePrimary));
      }
    }
    final BoxDecoration decoration = BoxDecoration(
      border: border,
      color: isSelected?AppColors.lumiBluePrimary:background,
