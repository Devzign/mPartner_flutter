...
import '../../../../presentation/widgets/something_went_wrong_screen.dart';
import '../../../network/api_constants.dart';


class GemTermConditionBottomSheet extends StatefulWidget {
  final bool firstAppearance;

  GemTermConditionBottomSheet({
    required this.firstAppearance
  });

  @override
  State<GemTermConditionBottomSheet> createState() =>
      _TermsConditionsBottomSheetState();
}

class _TermsConditionsBottomSheetState
    extends State<GemTermConditionBottomSheet> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      final response = await Requests.sendPostRequest(
          GemApiConstants.fetchTermConditionData,
          body
      );
      ...
      if (response is! DioException && response.statusCode == 200) {
        ...
        if (widget.firstAppearance) {
          Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                  builder: (context) => SomethingWentWrongScreen(onPressed: (){
                              ...
                              Column(
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Checkbox(
                                        value: isTermsAndConditionAccepted ?? false,
                                      ...
                                      GestureDetector(
                                        onTap: (){
                                          setState(() {
                                            if (isTermsAndConditionAccepted != null && isTermsAndConditionAccepted!) {
                                              isTermsAndConditionAccepted = false;
                                            } else if (isTermsAndConditionAccepted != null && isTermsAndConditionAccepted! == false) {
                                  ...
                                  // AcceptButton(
                                  //   isEnabled: isTermsAndConditionAccepted ?? false,
                                  //   onPressed: () {
                                  //     saveTermsConditionController.fetchSaveTermsCondition(termsConditionId);
                                  //     SharedPreferencesUtil.setShowSolorModuleDisclaimerAlert(isTermsAndConditionAccepted!);
